...
Part 1 covered [the bug](/posts/acode-cross-app-scripting-what-i-found/): an unescaped filename
...
tells you three things straight away:
...
That third one is what matters. The commit is four files, `+8/-4`, and adds `DOMPurify.sanitize()` at
three dialog sinks. It touches **no source**. `src/lib/checkFiles.js` still hands over the raw
filename. `helpers.errorMessage()` is byte-identical and still emits `<br>` on purpose.

A sink-side-only fix is a standing invitation. The dangerous value is still flowing raw, and only the
known exits got plugged. Every other exit that value reaches is an unfixed instance of the same bug.
...
`content://`. That last one is the whole attack surface here. It means another app can hand Acode a
file, and so hand Acode a filename.
...
About 20 hits. Small enough to read every one.
...
Constants are safe by construction, so stop reading. Interpolations need the value traced back to a
source. That single filter turns twenty sites into a handful.

Working through them:

| Site | Verdict |
|---|---|
| `src/dialogs/box.js:55` | Safe. All seven callers pass app-built HTML, and `showFileInfo.js` uses mustache, which auto-escapes. |
...
**Negative results are most of the work.** Twenty `innerHTML` sites, four with attacker-reachable data,
one exploitable. Being able to dismiss sixteen quickly is what lets the search finish.
...
The simplest way to prove the sink is to skip the malicious app entirely and just create a file with a
hostile name, assuming you can write somewhere Acode reads.
...
That evals every open file's contents. Our file is the payload, and the empty scratch tab evals to
nothing.
...
1. **The broken-image icon means the injection succeeded.** `innerHTML` parsed a real `<img>` element.
   If the filename had been escaped you'd see the literal string `<img src=x ...>` as text. Escaped
...
`editorManager.files.forEach(f=`, a syntax error, and everything after the `>` spilled out of the tag
as text.

The fix is a form containing no `>`:
...
Part 3 is the developer-facing half. Why the sink-side fix pattern keeps failing in this codebase, what
the durable fix looks like, and how to find this class of bug in your own hybrid app.
